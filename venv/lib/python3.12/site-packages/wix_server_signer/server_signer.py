...
class ServerSigner:
    ...
    def __init__(self, app_def_id: str, app_secret: str):
        """
        A class which creates a JWT signature in order to create GRPC calls within the Wix ecosystem
        For the Scala version please refer to: https://github.com/wix-platform/wix-framework/blob/master/crypto-modules/hoopoe-token/src/main/scala/com/wixpress/hoopoe/token/jwt/JwtSignedTokenEncoder.scala
    ...
    def _generate_payload(self, payload):
        ...
        attributes_to_add = {"exp": int((current_timestamp + timedelta(seconds=self.ONE_HOUR)).timestamp()),
                             "iat": int(current_timestamp.timestamp())}
        payload.update(attributes_to_add)
        return payload

    def decode_jwt(self, signature: bytes) -> dict:
        return jwt.decode(signature.replace(b'SRV.JWS.', b''),
                          self.app_secret,
                          algorithms='HS256')
